 ...
 *
 * public class ApplicationConfiguration {
 *     // Публичное статическое final поле — выбранный язык по умолчанию
 *     public static final String APPLICATION_DEFAULT_LANGUAGE;
 *
 *     // Статический блок выполняется один раз при загрузке класса.
 *     // Здесь мы инициализируем final-поле на основе переменной окружения.
 *     static {
 *         String envLang = System.getenv("APP_LANG"); // Пытаемся получить переменную окружения APP_LANG
 *         if (envLang != null) {
 *             APPLICATION_DEFAULT_LANGUAGE = envLang; // Если найдена — используем её
 *         } else {
 *             APPLICATION_DEFAULT_LANGUAGE = "en";     // Иначе — язык по умолчанию английский
 *         }
 *     }
 * }
 */