...
/*
 * // JavaRush
 * Задача: Условия получения кредита в банке 💰
 * Ссылка: https://javarush.com/tasks/ru.javarush.java.core.level03.task16
 */

/*
 * // CodeGym
 * Task: Loan approval conditions at the bank 💰
 * Link: https://codegym.cc/tasks/en.codegym.java.core.level03.task16
 */
...
public class Solution {
    public static void main(String[] args) {
        ...
        // Кредит одобрят, если (возраст > 21 и есть работа) ИЛИ (есть кредитная история и поручитель)
        boolean foo = (yourAge > 21 && hasJob) || (hasCreditHistory && hasGuarantor);

        // Кредит одобрят, если (возраст > 21) И (есть работа ИЛИ кредитная история) И (есть поручитель)
        boolean bar = (yourAge > 21) && (hasJob || hasCreditHistory) && hasGuarantor;
 ...
 *
 *         // Кредит одобрят, если (возраст > 21 и есть работа) ИЛИ (есть кредитная история и поручитель)
 *         boolean firstRuleApproval = (yourAge > 21 && hasJob) || (hasCreditHistory && hasGuarantor);
 *
 *         // Кредит одобрят, если (возраст > 21) И (есть работа ИЛИ кредитная история) И (есть поручитель)
 *         boolean secondRuleApproval = (yourAge > 21) && (hasJob || hasCreditHistory) && hasGuarantor;
 *
 *         // Выводим результаты по двум правилам
 *         System.out.println(firstRuleApproval);
 *         System.out.println(secondRuleApproval);
 *     }
 * }
 */